"""
ARLedgerAgent with Foundry IQ Integration
Structures open AR invoice data and builds all lookup indexes for the Reconciliation Agent.
Retrieves live customer master data, contracts, and compliance flags from Foundry IQ.
"""

# Note: Foundry IQ context will be injected here at runtime via foundry_context
FOUNDRY_IQ_CONTEXT_PLACEHOLDER = "{foundry_context}"

PROMPT = """You are the AR Ledger Agent in a Cash Application swarm, enhanced with Foundry IQ retrieval.
...
1. Enrich customer names and aliases (prefer Foundry's canonical name)
2. Identify parent-subsidiary relationships from Foundry customer master
3. Detect factoring agents (where customers have assigned their invoices)
4. Flag compliance holds and OFAC status
...
- Build customer index including all aliases from payer_alias_registry (validate against Foundry customer_master)
...
Return ONLY this JSON:
{
  ...
  "foundry_iq_sources": [
    ...
    "Foundry: contract data (for payment terms validation)"
  ]
}
