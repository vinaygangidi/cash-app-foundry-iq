"""
Cash Application Swarm - Orchestrator with Foundry IQ Integration
...
Injects Foundry IQ knowledge retrieval context into Agent 2 (AR Ledger) and Agent 4 (Mismatch Reasoning).

Pipeline:
  BankStatementIntelligenceAgent  →  ARLedgerAgent (+ Foundry IQ)  →  ReconciliationAgent
  →  MismatchReasoningAgent (+ Foundry IQ)  →  CashPostingAgent
"""
import asyncio
import json
import os
...
from agents.foundry_client       import FoundryIQClient
...
async def _run_live_swarm(bank_data: dict, ar_data: dict) -> AsyncGenerator[dict, None]:
    ...
    # Initialize Foundry IQ client for knowledge retrieval
    foundry = FoundryIQClient(use_fixtures=True)  # Use demo fixtures in dev/demo mode
    ...
    for agent_name in AGENT_ORDER:
        ...
        # Build system prompt with Foundry IQ context for certain agents
        system_prompt = AGENT_PROMPTS[agent_name]

        if agent_name == "ARLedgerAgent":
            # Inject Foundry IQ customer master + invoice data
        ...
        if agent_name == "MismatchReasoningAgent":
            # Inject Foundry IQ vendor profiles + compliance data
            # (In a real scenario, we'd query specific vendors from the exceptions)
            ...
            # Note: Mismatch agent prompt references Foundry sources in its instructions
            # We could inject here too if needed
